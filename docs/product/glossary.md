...
|---|---|---|
| SDD | `spec-driven development` | Desarrollo dirigido por especificación: constitución → spec → clarificación → plan → tareas → implementación → validación → cambio. Guía en `docs/sdd/README.md`. |
| Constitución | `constitution` | Principios innegociables del producto, cada uno con su forma de verificarse. `docs/constitution.md`. |
...
| RNF | `non-functional requirement` | Requisito no funcional transversal (sesión, rendimiento, accesibilidad…), numerado `RNF-n` en un catálogo único: `docs/product/requisitos-transversales.md`. |
...
| Plan | `plan` | El CÓMO de una spec: módulos, datos, contratos, decisiones justificadas y estrategia de tests. Solo tras decidir el stack (ADR-0002). |
