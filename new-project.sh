...
set -euo pipefail
NAME="${1:?nombre-kebab}"; DEST="${2:?ruta-destino}"; DESC="${3:-{{DESCRIPCIÓN EN 2–3 FRASES: qué es, para quién, qué problema resuelve}}}"
...
find "$TARGET" -type f -name "*.md" -print0 | xargs -0 sed -i.bak \
  -e "s/{{PROYECTO}}/$NAME/g" \
  -e "s/{{FECHA}}/$TODAY/g" \
  -e "s|{{DESCRIPCIÓN EN 2–3 FRASES: qué es, para quién, qué problema resuelve}}|$DESC|g"
